...
def generate(args_list):
    ...
    for step in range(1, args.n_iter + 1):
        ...
        if step % 500 == 0:
            ...
            print(f"[{step}] current energy: {avg_energy:.4f}")
            wandb.log({"avg_energy": avg_energy})
    
    wandb.log({"final_avg_energy": energy.mean().item()})
    ...
    wandb_logdir = os.path.join("../log", wandb.run.name)  # 또는 wandb.run.id
    ...
    for i, object_code in enumerate(object_code_list):
        ...
        np.save(os.path.join(wandb_logdir, object_code + '.npy'), data_list, allow_pickle=True)
...
if __name__ == '__main__':
    ...
    ### Wandb setting ###
    run = wandb.init(
        project="DexGraspNet_KISTAR",
        name=datetime.now().strftime("%m%d_%H%M-KISTAR"),
        # monitor_gym=False, # ?
        config={
            # "seed": args.seed,
            # "n_contact": args.n_contact,
            # "max_total_batch_size": args.max_total_batch_size,
            # "batch_size_each": args.batch_size_each,
            # "n_iter": args.n_iter,
            # "switch_possibility": args.switch_possibility,
            # "mu": args.mu,
